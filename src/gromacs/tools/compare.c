...
static void cmp_pulse(FILE *fp, const char *s, t_efield c1[DIM], t_efield c2[DIM], real ftol, real abstol)
{
    int  i, m;
    char buf[256];
    ...
    for (m = 0; (m < DIM); m++)
    {
        ...
        for (i = 0; (i < min(c1->n, c2->n)); i++)
        {
            cmp_real(fp, buf, i, c1->E0[i], c2->E0[i], ftol, abstol);
            cmp_real(fp, buf, i, c1->sigma[i], c2->sigma[i], ftol, abstol);
            cmp_real(fp, buf, i, c1->t0[i], c2->t0[i], ftol, abstol);
            cmp_real(fp, buf, i, c1->omega[i], c2->omega[i], ftol, abstol);
        }
    }
}
...
static void cmp_inputrec(FILE *fp, t_inputrec *ir1, t_inputrec *ir2, real ftol, real abstol)
{
    ...
    cmp_pulse(fp, "et", ir1->et, ir2->et, ftol, abstol);
}
