...
static void decode_pulse(char *s, t_efield *cosine)
{
    char   *t;
    char    format[STRLEN], f1[STRLEN];
    double  E0, sigma, t0, omega;
    int     i;
    ...
    if (strlen(t))
    {
        ...
        {
            snew(cosine->E0, cosine->n);
            snew(cosine->omega, cosine->n);
            snew(cosine->t0, cosine->n);
            snew(cosine->sigma, cosine->n);

            sprintf(format, "%%*d");
            for (i = 0; (i < cosine->n); i++)
            {
                strcpy(f1, format);
                strcat(f1, "%lf%lf%lf%lf");
                if (sscanf(t, f1, &E0, &omega, &t0, &sigma) < 4)
                {
                    gmx_fatal(FARGS, "Invalid input for pulse electric field shift: '%s'", t);
                }
                cosine->E0[i]   = E0;
                cosine->sigma[i] = sigma;
                cosine->t0[i]   = t0;
                cosine->omega[i] = omega;

                strcat(format, "%*lf%*lf%*lf%*lf");
            }
        }
    }
    sfree(t);
              ...
              warninp_t wi)
{
    ...
    decode_pulse(is->efield_xt, &(ir->et[XX]));
    decode_cos(is->efield_y, &(ir->ex[YY]));
    decode_pulse(is->efield_yt, &(ir->et[YY]));
    decode_cos(is->efield_z, &(ir->ex[ZZ]));
    decode_pulse(is->efield_zt, &(ir->et[ZZ]));
