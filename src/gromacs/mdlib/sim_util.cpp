...
static void calc_f_el(FILE *fp, int  start, int homenr,
                      real charge[], rvec f[], rvec x[], rvec v[],
                      t_cosines Ex[], t_efield Et[], double t)
{
    rvec Ext, Mxt;
    real t0;
    int  np, i, m;
	...
	Mxt[m]=0;
        if (Et[m].n > 0)
        {
	   // determine time point of pulse
	   if (Et[m].n > 1) {
	   	for (int jj=0; (jj<Et[m].n-1); jj++)
			{
			if(((Et[m].t0[jj+1]+Et[m].t0[jj])- 2*t) < 0)
				{np=jj+1;}
			}
	   	}
            ...
            {
                t0     = Et[m].t0[np];
                Ext[m] = Et[m].E0[np]*cos(Et[m].omega[np]*(t-t0))*exp(-sqr(t-t0)/(2.0*sqr(Et[m].sigma[np])));
            }
            else
            {
                Ext[m] = cos(Et[m].omega[np]*t);
            }
        }
        else
        {
		...
		int m1=(m+1)%3;
		int m2=(m+2)%3;
		Mxt[m] = FIELDFAC*(v[i][m1]*Ex[m2].phi[0]-v[i][m2]*Ex[m1].phi[0]);
		//printf("MXT[M]:, %d %d  %d  %d  %10g %10g %10g  %10g %10g %10g\n",np,  m, m1, m2,  Ex[m].phi[0],  Ex[m1].phi[0], Ex[m2].phi[0], Mxt[m], Ext[m], FIELDFAC);
                f[i][m] += charge[i]*(Ext[m]+Mxt[m]);
            }
        }
        else
        {
    ...
    if (fp != NULL)
    {
        fprintf(fp, "%10g  %10g  %10g  %10g %10g %10g %d\n", t,
                Ext[XX]/FIELDFAC, Ext[YY]/FIELDFAC, Ext[ZZ]/FIELDFAC), Mxt[XX]/FIELDFAC, Mxt[YY]/FIELDFAC, np;
        //printf("%10g  %10g  %10g  %10g %lf %lf %d\n", t,
        //        Ext[XX]/FIELDFAC, Ext[YY]/FIELDFAC, Ext[ZZ]/FIELDFAC), Mxt[0], FIELDFAC, np;
    }
}
...
void do_force_cutsVERLET(FILE *fplog, t_commrec *cr,
                         ...
                         matrix box, rvec x[], rvec v[], history_t *hist,
                         rvec f[],
                         tensor vir_force,
                         t_mdatoms *mdatoms,
    ...
    if (bDoForces)
    {
        if (IR_ELEC_FIELD(*inputrec))
        {
            ...
            calc_f_el(MASTER(cr) ? field : NULL,
                      start, homenr, mdatoms->chargeA, fr->f_novirsum, x, v,
