...
static void bc_pulse(const t_commrec *cr, t_efield *cs)
{
    block_bc(cr, cs->n);
    snew_bc(cr, cs->E0, cs->n);
    snew_bc(cr, cs->omega, cs->n);
    snew_bc(cr, cs->t0, cs->n);
    snew_bc(cr, cs->sigma, cs->n);
    if (cs->n > 0)
    {
        nblock_bc(cr, cs->n, cs->E0);
        nblock_bc(cr, cs->n, cs->omega);
        nblock_bc(cr, cs->n, cs->t0);
        nblock_bc(cr, cs->n, cs->sigma);
    }
}
