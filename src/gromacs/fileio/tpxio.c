                        ...
                        int file_version, real *fudgeQQ)
{
    ...
    for (j = 0; (j < DIM); j++)
    {
        ...
        if (bRead)
        {
            ...
            snew(ir->et[j].E0,  ir->et[j].n);
            snew(ir->et[j].omega, ir->et[j].n);
            snew(ir->et[j].t0,  ir->et[j].n);
            snew(ir->et[j].sigma, ir->et[j].n);
        }
        gmx_fio_ndo_real(fio, ir->ex[j].a,  ir->ex[j].n);
        gmx_fio_ndo_real(fio, ir->ex[j].phi, ir->ex[j].n);
        gmx_fio_ndo_real(fio, ir->et[j].E0,  ir->et[j].n);
        gmx_fio_ndo_real(fio, ir->et[j].omega, ir->et[j].n);
        gmx_fio_ndo_real(fio, ir->et[j].t0,  ir->et[j].n);
        gmx_fio_ndo_real(fio, ir->et[j].sigma, ir->et[j].n);
